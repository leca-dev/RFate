...
##'   \item{Champsaur_PARAM}{all necessary files to build the \code{FATE} 
...
##' \strong{\cr#########################################################\cr\cr}
##' 
##' A \code{list} object with 15 elements to help building the Plant Functional 
##' Group :
##'   
##'   \describe{
...
##'     }
##'     }
##'     \item{PFG.traits}{\code{data.frame} of dimension \code{14 x 13} \cr
...
.loadData = function(data.name, format.name)
{
  ...
  .testParam_notInValues.m("data.name", data.name, c("Champsaur_PFG"))
