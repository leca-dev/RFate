...
##' 
##' @description To compare observations and simulations, this function computes 
##' confusion matrix between observation and prediction and then compute the TSS 
##' for each habitats.
##' 
##' @param output.path a \code{string} containing the access path to the for the folder where output files
##' will be created.
##' @param RF.model random forest model trained on observed abundance data (output of \code{train_RF_habitat}
##' function)
##' @param predict.all.map \code{logical}. \cr
##' If TRUE, the script will predict habitat for the whole map.
##' @param sim a \code{string} containing the name of the single simulation to validate.
...
##' @param habitat.whole.area.df a \code{data.frame} built in \code{POST_FATE.validation} with 3 columns : 
...
##' @param list.strata if abundance file is defined by strata : a character vector containing \code{FATE} 
...
##' @param perStrata \code{logical}. \cr 
##' If \code{TRUE}, PFG abundance is defined by strata. 
