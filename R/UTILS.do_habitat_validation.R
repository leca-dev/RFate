...
do_habitat_validation <- function(output.path, RF.model, predict.all.map, sim, simu_PFG, habitat.whole.area.df, list.strata, perStrata)
{
  ...
  if(length(setdiff(FATE.PFG,RF.PFG)) > 0) {
    cat(paste0("\n Warning : The PFG used to train the RF algorithm are not the same as the PFG used to run FATE ! The PFG", setdiff(FATE.PFG,RF.PFG), "will be deleted"))
    FATE.PFG = RF.PFG
  }else if(length(setdiff(RF.PFG,FATE.PFG)) > 0){
    cat(paste0("\n Warning : The PFG used to train the RF algorithm are not the same as the PFG used to run FATE ! The PFG", setdiff(RF.PFG,FATE.PFG), "will be deleted"))
    RF.PFG = FATE.PFG
  }
